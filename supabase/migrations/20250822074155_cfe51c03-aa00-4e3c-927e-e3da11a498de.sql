-- Fix function overloading conflict by dropping old version and creating clean new one
DROP FUNCTION IF EXISTS public.reorder_task(uuid, text, integer);
DROP FUNCTION IF EXISTS public.reorder_task(uuid, public.task_status, integer);

-- Create single clean function with proper enum type
...
BEGIN
  ...
  RAISE NOTICE 'Task % moved to % with order_index %', p_task_id, p_new_status, new_order_index;
END;
$$;